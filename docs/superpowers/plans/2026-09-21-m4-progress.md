...
> ⚠️ **实施时的顺序修正（2026-09-21，实测后补记）**
>
> Task 6 的代码要 `router.push({ pathname: '/exercise/[id]' })`，而 expo-router 的 typedRoutes
> 类型是**预生成**在 `.expo/types/router.d.ts` 里的（`tsconfig.json` 用 `.expo/types/**/*.ts`
> 把它 include 进去了）。那条路由字面量要等 Task 7 建出 `app/exercise/[id].tsx` 才会出现 ——
> 旧文件里只有 `/history/[id]`、`/session/[id]`、`/session/summary/[id]`。
>
> **所以实际执行顺序是：Task 7 → 重新生成路由类型 → Task 6。**
>
> 重新生成的办法（实测有效）：
>
> ```powershell
> . .superpowers\build-env.ps1
> Remove-Item Env:\CI -ErrorAction SilentlyContinue   # CI=1 时 Metro 进 CI 模式，不会生成类型
> Set-Location C:\ccproject\gym
> npx expo start                                       # 后台起，等约 25 秒
> ```
>
> 等到 `.expo/types/router.d.ts` 的 mtime 更新、且里面出现 `exercise` 字样，就可以把 dev server 关掉。
> 这个文件在 `.gitignore` 里（`.expo/`），不影响提交。
...
- [x] **Step 5: 顺带把 M5 备份的真机验收做掉**

M5 的代码早就完成了，但 `2026-09-16-m5-backup.md` 的 Task 4 那一整节**从来没在真机上验过**（导出 → 分享面板 → 清除数据 → 导入 → 历史恢复）。它共用同一次打包与同一部手机，一起做掉：

- [x] 导出备份 → 系统分享面板弹出 → 存到手机
- [x] 记录当前训练场数
- [x] `adb shell pm clear com.ccproject.gymtracker` → 重开，历史应为空
- [x] 导入刚才那份备份 → 确认框里场数正确 → 导入后历史恢复
- [x] 对比导入前后的训练详情完全一致（次数数组、组数）
- [x] 故意选一个非 JSON 文件导入 → 提示「不是有效的 JSON」，且库里数据不变
...
8 个 Task 由独立 subagent 逐个执行。**这次 subagent 一律不碰 git** —— 只写文件、跑测试，提交全部由主 agent 在每个 Task 之间完成。上一轮「训练生命周期」有两个 subagent 并行时撞了共享 index、两条改动挤进同一个提交，这次从根上避开了。

| Task | 提交 | 内容 |
|---|---|---|
| 1 | `363f2b8` | 新增 `react-native-svg@15.15.4`（**主 agent 自己做**：只有两条命令，而 Expo CLI 在这个环境里的坑这一会话已经踩过） |
| 2 | `76e1bba` | `lib/chartGeometry.ts` + 9 条测试 |
| 3 | `b1eef03` | `repositories/progressRepo.ts` + 7 条测试 |
| 4 | `5b2148b` | `domain/progress.ts` + 15 条测试 |
| 7 | `5f2c9c9` | `app/exercise/[id].tsx` + 注册路由（**先于 Task 6**，原因见下） |
| 5 | `c9bb2da` | `components/TrendChart.tsx` |
| 6 | `8e34246` | 「进步」页动作列表 |
...
**1. Task 4 的测试与实现自相矛盾。** 实现（连同它的两处注释）写的是「同重量时取次数多的那一组」，测试却断言并列取先出现的那个（`40×12 / 40×15` 期望 `reps: 12`）。两条规则不可能同时满足。subagent 没有自行改断言、也没有为了变绿去改实现，而是把红灯交回来 —— 处置正确。**改的是测试那一行**（期望值改成 15）：同样重量多举 3 次显然是更好的一组，实现的语义才是对的。

**2. Task 6/7 的顺序必须互换。** Task 6 要 `router.push({ pathname: '/exercise/[id]' })`，而 expo-router 的 typedRoutes 类型是预生成在 `.expo/types/router.d.ts` 里的（`tsconfig.json` 用 `.expo/types/**/*.ts` 把它 include 了），那条字面量要等 Task 7 建出路由文件才会有。另外这个文件在 `CI=1` 下**不会**重新生成（Metro 进 CI 模式），必须 `Remove-Item Env:\CI` 之后再跑 `npx expo start`。**实际顺序：Task 7 → 重新生成路由类型 → Task 6。**

顺带一个反直觉的实测结果：**Task 7 本身并不需要这次重新生成** —— 详情页从不引用它自己的路由字面量，`<Stack.Screen name="exercise/[id]">` 在当前 expo-router 版本下也没被收窄到生成的路由联合类型上，所以它的 tsc 在没有该类型的条件下照样是绿的。真正需要它的是 Task 6。

### 不需要真机也做掉的一项验证

`tsc` 与 `jest` 都覆盖不到「整个 App 到底打不打得成包」。所以在真机之前，用 Metro 打了一次完整的 Android dev bundle：

```
GET http://localhost:8081/.expo/.virtual-metro-entry.bundle?platform=android&dev=true
→ HTTP 200，7,038,298 字节
```

并按字节确认新代码确实进了 bundle。**注意 Metro 把中文串转成了 `\uXXXX` 大写十六进制转义**，必须用转义形式搜（小写搜不到）：

| 搜到的转义串 | 原文 | 出处 |
|---|---|---|
| `\u518D\u7EC3\u4E00\u6B21\u5C31\u80FD\u770B\u5230\u8D70\u52BF` | 再练一次就能看到走势 | 详情页单点提示 |
| `\u603B\u5BB9\u91CF\uFF08\u91CD\u91CF \xD7 \u6B21\u6570\uFF09` | 总容量（重量 × 次数） | 详情页容量卡说明 |
| `\u8FD8\u6CA1\u6709\u8BAD\u7EC3\u8BB0\u5F55` | 还没有训练记录 | 进步页空态 |

**bundle 里出现的 `Unable to resolve module` 与 `TransformError` 字样都是 RN 自己的错误模板源码**（例如 `UNABLE_TO_RESOLVE_MODULE_ERROR_FORMAT = /(?:\w )?Unable to resolve module (.*) from/`），不是真错误 —— 拿字符串搜 bundle 时别被这两句骗了。
